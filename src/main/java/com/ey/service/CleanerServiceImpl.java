...
public class CleanerServiceImpl implements CleanerService {
    ...
    @Override
    public ResponseEntity<?> getCleanerBookingsByDate(Long cleanerId, LocalDate date) {
       ...
       List<Booking> bookings =
               bookingRepo.findByCleanerIdAndServiceDate(cleanerId, date);
       if (bookings.isEmpty()) {
           return ResponseEntity.ok("No bookings found for this date");
       }
       List<CleanerBookingDetailsResponse> responses = new ArrayList<>();
       for (Booking booking : bookings) {
           responses.add(BookingMapper.toCleanerBookingResponse(booking));
